...
echo -e "\\x1b[2m│\\x1b[0m  \\x1b[97mVibeGuard\\x1b[0m · Pre-Push Analysis           \\x1b[2m│\\x1b[0m"
echo -e "\\x1b[2m╰──────────────────────────────────────────╯\\x1b[0m"
echo ""
...
while read -r local_ref local_sha remote_ref remote_sha; do
  ...
  echo -e "\\x1b[90m→\\x1b[0m  \\x1b[97m\$local_branch\\x1b[0m \\x1b[90m→\\x1b[0m \\x1b[97m\$remote_branch\\x1b[0m"
...
echo ""
if [ \$FAILED -eq 0 ]; then
  echo -e "\\x1b[32m✓  VibeGuard analysis passed\\x1b[0m"
  exit 0
else
  echo -e "\\x1b[31m✕  Push aborted — VibeGuard analysis failed\\x1b[0m"
  exit 1
fi
`;
}
...
function generatePowerShellHook(vibeguardEntry: string): string {
  ...
  return [
    ...
    `Write-Host ("${e}[2m│${e}[0m  ${e}[97mVibeGuard${e}[0m · Pre-Push Analysis           ${e}[2m│${e}[0m")`,
    `Write-Host ("${e}[2m╰──────────────────────────────────────────╯${e}[0m")`,
    'Write-Host ""',
    "",
    '$failed = $false',
    ...
    "",
    `    Write-Host ("${e}[90m→${e}[0m  ${e}[97m$localBranch${e}[0m ${e}[90m→${e}[0m ${e}[97m$remoteBranch${e}[0m")`,
    "",
    ...
    'Write-Host ""',
    "if (-not $failed) {",
    `  Write-Host ("${e}[32m✓  VibeGuard analysis passed${e}[0m")`,
    "  exit 0",
    "} else {",
    `  Write-Host ("${e}[31m✕  Push aborted — VibeGuard analysis failed${e}[0m")`,
    "  exit 1",
    "}",
    "",
  ].join("\n");
...
export function installHook(): void {
  ...
  if (fs.existsSync(bashHookPath)) {
    ...
    if (!existing.includes("VibeGuard")) {
      ...
      ui.warn(`Existing hook backed up → ${backupPath}`);
    }
  }

  fs.writeFileSync(bashHookPath, bashContent, { mode: 0o755 });
  ui.ok(`Hook installed → ${bashHookPath}`);
  ...
  if (os.platform() === "win32") {
    ...
    ui.muted(`PowerShell fallback → ${psHookPath}`);
